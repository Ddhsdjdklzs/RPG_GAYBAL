...
const j = '!';
const FS = FileStream;
const Path = '/sdcard/RPG/', UserListPath = '/sdcard/RPG/UserList.txt', RPGItemPath1 = '/sdcard/Pictures/RPGItemList1.json';
const progress = {};
...
let ItemLists1_Data = JSON.parse(FS.read(RPGItemPath1));
...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName, a, b, c, d){
    ...
    //허용 접두어 추가
    if(!msg.startsWith(j) && isNaN(msg)) return;
    let p = user(userId);
    ...
    if(msg == j + '광질'){
        ...
        let FilteredMinerals = ItemLists1_Data.Ores[RankResult];
