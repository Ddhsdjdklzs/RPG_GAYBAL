...
function response(room, msg, sender, isGroupChat, replier, imageDB, packageName, a, b, c, d){
    ...
    if(msg == j + '튜토리얼'){
        ...
        if(p.TutAmt > 0){
            ...
            reply('시스템 메세지 : 대답 선택은 해당 대답의 번호를 입력하시면 됩니다.');
            progress[userId] = '2';
        }
        save(userId, p);
    }
    ...
    switch(progress[userId]){
        ...
        case '6' : {
            if(msg == '!가방'){
                ...
                reply('자 그럼 이렇게 자네가 캔 광물과 나무를 한번에 볼 수 있다네. 그럼 돌을 한번 팔아보게나. "!판매 1"이라고 입력해보게.');
                reply('시스템 메세지 : "!판매 1"을 입력하세요');
            }
        } break;
        ...
        case '7' : {
            if(msg == '!판매 1'){
                progress[userId] = '8';
                reply('사용자님이 [일반] 돌을 판매하셨습니다.\n[200$ 획득]');
                reply('허허 대단하구만, 앞으로 그렇게 "!판매 "를 입력한 후 판매하고 싶은 아이템 번호를 적으면 될걸세. 이제 미션에 대해 알려주겠다네.\n\n1) 알겠어');
            }
        } break;
        ...
        case '8' : {
            if(msg == '1'){
                progress[userId] = '9';
                reply('미션을 진행할 경우 보상을 얻고, NPC들과 친해질 수 있을 것이라네. 정말 드물지만 가끔 희귀 칭호도 발견할 수 있으니 열심히 해보게나.\n\n1) 칭호는 뭐야?');
            }
        } break;
        ...
        case '9' : {
            if(msg == '1'){
                progress[userId] = '10';
                reply('이 게임에는 어떠한 행동을 해야만 얻을 수 있는 희귀 칭호들이 많다네. 도감을 통해서도 알 수 있겠지만, 칭호에는 4가지 등급이 있다네. 각각 [일반], [특이], [전설], [영원] 이지. 특히 [전설] 칭호부터는 칭호마다 고유 능력이 있으니 열심히 모아보게나.\n\n1) 도감은 뭐야?');
            }
        } break;
        ...
        case '10' : {
            if(msg == '1'){
                progress[userId] = '11';
                reply('도감은 어느 정신나감 작자의 손에서 태어난 시스템이라네. 거기엔 이 게임의 모든 아이템과 칭호와 관련된 정보들이 있다네. "!도감 "을 입력한 후 아이템 이름이나 키워드를 입력하여 그 정보를 열람할 수 있으나, 지금은 하지 말게나. 시간이 없으니 말일세.\n\n1) 알겠어');
            }
        } break;
        ...
        case '11' : {
            if(msg == '1'){
                progress[userId] = '12';
                reply('이제 스토리에 대해 설명해주겠네. 스토리는 이 세계의 진실을 알 수 있는 열쇠라네. 스토리를 모두 봐야만 이 세계의 진실을 알 수 있다네. 하지만 스토리를 보기 위해서는 돈이 필요하다네. 또한 스토리를 들려주는 각각의 NPC가 원하는 바를 이루어 주어야 한다네.\n\n1) 그렇군');
            }
        } break;
        ...
        case '12' : {
            if(msg == '1'){
                progress[userId] = '13';
                reply('마지막일세, 일반 곡괭이와 도끼로는 높은 등급의 광물 혹은 나무를 얻기는 어려울걸세. 하지만 곡괭이와 도끼를 강화해 그 확률을 높이는 방법이 있다네.\n\n1) 확률이 어떻게 되는데?\n2) 알겠어');
            }
        } break;
        ...
        case '13' : {
            if(msg == '1'){
                progress[userId] = '14';
                reply('우선 아이템의 등급부터 알아야 한다네. 알려진 바로는 9개가 전부일세. 각각 [일반], [희귀], [초희귀], [영웅], [전설], [초월], [영원], [불멸], [무한]이 있다네. 기본 곡괭이로는 [일반] - 40%, [희귀] - 32%, [초희귀] - 18%, [영웅] - 4%, [전설] - 3%, [초월] - 2%, [영원] - 0.9%, [불멸] - 0.08%, [무한] - 0.019% 라네.\n\n1) 더 높은 등급은 없어?');
            }
            if(msg == '2'){
                progress[userId] = '15';
                reply('곡괭이와 도끼 강화는 강화재료와 돈을 소모한다네. 강화재료는 광물이나 나무를 재련소에서 재련하면 될걸세.\n\n1) 재련소는 뭐야?');
            }
        } break;
        ...
        case '14' : {
            if(msg == '1'){
                progress[userId] = '19';
                reply('그건 나도 모른다네. 하지만 자네가 노력한다면 찾아낼지도 모르겠군. 노력해보게나.\n\n1) 알겠어');
            }
        } break;
        ...
        case '15' : {
            if(msg == '1'){
                progress[userId] = '16';
                reply('허허, 그건 다른 이들에게 묻거나 "!도움말"을 입력해 답을 찾아보게나. 그것이 자네의 길잡이가 되어줄 것이라네.\n\n1) 알겠어');
            }
        } break;
        ...
        case '16' : {
            if(msg == '1'){
                ...
                reply('이제 이 아이템을 들고 모험을 시작해보게나. 행운을 빈다네.\n\n1) 다시 설명해줘\n2) 종료');
                p.TutAmt += 1;
                p.Tut = true;
                save(userId, p);
            }
        ...
        case '17' : {
            if(msg == '1'){
                progress[userId] = '3';
                if(p.TutAmt == 1){
                    reply('자네 안듣고 뭐했나? 이번엔 제대로 듣게나.');
                    reply('[한스] : 다시 설명하겠네. 이게임의 목적은 세계의 진실을 알아내는 것이라네. 그게 뭐냐고? 허허, 나도 잘은 모른다네. 그건 오직 『개발자』님의 영역이니 말이야.\n\n1) 그럼 어떻게 하는지 알려줘.\n2) 개발자가 누구야?');
                }
                else if(p.TutAmt == 2){
                    reply('자네 지금 장난하는겐가. 마지막일세.');
                    reply('[한스] : 다시 설명하겠네. 이게임의 목적은 세계의 진실을 알아내는 것이라네. 그게 뭐냐고? 허허, 나도 잘은 모른다네. 그건 오직 『개발자』님의 영역이니 말이야.\n\n1) 그럼 어떻게 하는지 알려줘.\n2) 개발자가 누구야?');
                }
                else if(p.TutAmt == 3){
                    reply('자네! 정말 안듣고 있구만. 이 이상 나를 화나게 하지 말게나.');
                    reply('칭호 [전설을 화나게 한 남자]를 획득하셨습니다.');
                    reply('[한스] : 다시 설명하겠네. 이게임의 목적은 세계의 진실을 알아내는 것이라네. 그게 뭐냐고? 허허, 나도 잘은 모른다네. 그건 오직 『개발자』님의 영역이니 말이야.\n\n1) 그럼 어떻게 하는지 알려줘.\n2) 개발자가 누구야?');
                } else {
                    reply('자네 해보자는 건가? 좋다. 누가 끝까지 가나 보세.');
                    reply('[한스] : 다시 설명하겠네. 이게임의 목적은 세계의 진실을 알아내는 것이라네. 그게 뭐냐고? 허허, 나도 잘은 모른다네. 그건 오직 『개발자』님의 영역이니 말이야.\n\n1) 그럼 어떻게 하는지 알려줘.\n2) 개발자가 누구야?');
                }
            }
            if(msg == '2'){
